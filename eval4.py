...
for feature in features:
    ...
    if tree_predictions:
        ...
        # Store the importance metrics directly
        importance_scores[feature] = importance_metrics
    else:
        # If no predictions are available, set importance score to 0 or some default value
        importance_scores[feature] = {"mean_difference": 0}
...
for feature in features:
    ...
    for level in unique_levels:
        # Modify the dataset by removing or altering the level
        X_modified = X.copy()
        X_modified[feature] = X_modified[feature].apply(lambda x: np.nan if x == level else x)
        ...
        # Compute metrics for the predictions with the modified dataset
        level_metrics = compute_metrics(y, final_predictions)
        
        # Store the level importance metrics directly
        level_importance_scores[feature][level] = level_metrics

# Print level importance scores
for feature, levels in level_importance_scores.items():
    for level, score in levels.items():
        print(f"Importance of {feature} level {level}: {score}")
