...
# Compute the mean difference in tricks from the final_predictions and all of the subsets
mean_differences = {}

for feature, levels in level_predictions.items():
    mean_differences[feature] = {}
    for level, preds in levels.items():
        mean_diff = (final_predictions - preds).mean()
        mean_differences[feature][level] = mean_diff

# Print the mean differences for each level of each feature
for feature, levels in mean_differences.items():
    for level, mean_diff in levels.items():
        print(f"Mean difference for {feature} = {level}: {mean_diff}")
